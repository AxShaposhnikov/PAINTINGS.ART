...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_acordion__WEBPACK_IMPORTED_MODULE_9__["default"])('.accordion-heading');
...
var acordion = function acordion(triggerSelector) {
  var btns = document.querySelectorAll(triggerSelector);

  var addActivityClass = function addActivityClass(item) {
  ...
  var removeActivityClass = function removeActivityClass(item) {
  ...
  btns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      if (this.classList.contains('active-style')) {
        btns.forEach(function (btn) {
          return removeActivityClass(btn);
        });
      } else {
        btns.forEach(function (btn) {
          return removeActivityClass(btn);
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunction = function calcFunction(response) {
  ...
  sizeBlock.addEventListener('change', function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResource"])('assets/calcDb.json').then(function (response) {
      return calcFunction(response.values);
...
var drop = function drop() {
  ...
  function unhighlight(item) {
    ...
    } else if (item.closest('.file_upload_post')) {
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      ...
      if (input.closest('.file_upload_post')) {
        console.log('Это ОНО!'); //Написать код отправки изображения на сервер

        Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["postData"])('assets/server.php', input.files).then(function (response) {
...
var filter = function filter() {
  ...
  var hideActive = function hideActive() {
    return items.forEach(function (item) {
      return item.classList.remove('active');
    });
  };

  var showActive = function showActive(item) {
    return item.classList.add('active');
  };

  function clickOnTab(btn) {
  ...
  ;
  clickOnTab(btnAll, markAll);
  clickOnTab(btnLovers, markLovers);
  clickOnTab(btnChef, markChef);
  clickOnTab(btnGirl, markGirl);
  clickOnTab(btnGuy, markGuy);
  clickOnTab(btnGrandmother);
  clickOnTab(btnGranddad);
};
...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      console.log(api);
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData).then(function (response) {
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    var distroy = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        windows.forEach(function (item) {
          item.style.display = 'none';
          item.classList.add('animated', 'fadeIn'); // Анимация для модальных окон
        });
...
var pictureSize = function pictureSize() {
  var sizePicturesBlocks = document.querySelectorAll('.sizes-block');
  sizePicturesBlocks.forEach(function (block) {
    block.addEventListener('mouseenter', function () {
      block.querySelector('img').src = block.querySelector('img').src.slice(0, block.querySelector('img').src.length - 4) + "-1.png";
      block.querySelectorAll('p').forEach(function (pTag) {
    ...
    block.addEventListener('mouseleave', function () {
      ...
      block.querySelectorAll('p').forEach(function (pTag) {
        return pTag.style.display = 'block';
      });
    });
  });
};
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  function showErrorMsg(err) {
    ...
    errMsgBox.textContent = 'Ошибка соединения с базой данных: ' + err.message;
    errMsgBox.style.cssText = "text-align: center; font-size: 20px; padding: 10px 10px; margin: 20px 20px;";
    document.querySelector(wrapper).appendChild(errMsgBox);
  }
